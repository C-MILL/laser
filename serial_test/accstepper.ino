...
String command; // variable for commands
...
{  
  ...
  if (newData == true) // if we received something (see above)
  {
    ...
    // START - OPEN
    if (command.startsWith("p")) // OPENING stepper 1
    {
      runallowed = true; // allow running
      receivedMMdistance = command.substring(1).toInt(); // value for the steps
      receivedDelay = command.substring(2).toInt(); // value for the speed
    ...
    if (command.startsWith("b")) // OPENING stepper 2
    {
      runallowed = true; // allow running
      receivedMMdistance = command.substring(1).toInt(); // value for the steps
      receivedDelay = command.substring(2).toInt(); // value for the speed
    ...
    // START - CLOSE
    if (command.startsWith("c")) // CLOSING - Rotates the motor in the opposite direction as opening
    {
      runallowed = true; // allow running
      receivedMMdistance = command.substring(1).toInt(); // value for the steps
      receivedDelay = command.substring(2).toInt(); // value for the speed
    ...
    // STOP - STOP
    if (command == "n") // immediately stops the motor
    {
    ...
    // SET ACCELERATION
    if (command.startsWith("a")) // Setting up a new acceleration value
    {
